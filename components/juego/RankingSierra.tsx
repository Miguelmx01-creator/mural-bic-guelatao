...
const PODIO_MEDALS = ['🥇', '🥈', '🥉'];
const PODIO_HEIGHTS = [100, 80, 65]; // px mínimos de la columna del podio
...
export default function RankingSierra({ jugadorActual, onVolver }: Props) {
  ...
  const podioOrder   = [1, 0, 2]; // índice real en ranking para colores/medallas
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2
          className="font-game text-[#F2C14E] text-2xl tracking-wider"
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide px-4 py-6">
        ...
        {podio.length > 0 && (
          <div className="flex items-end justify-center gap-2 mb-8 px-2">
            {podioDisplay.map((jugador, vi) => {
              ...
              const ri = podioOrder[vi]; // índice real (0=1°, 1=2°, 2=3°)
              const color  = PODIO_COLORS[ri];
              const medal  = PODIO_MEDALS[ri];
              const height = PODIO_HEIGHTS[ri];
              const esActual = jugador.id === jugadorActual.id;
              ...
              return (
                <div key={jugador.id} className="flex flex-col items-center gap-1 w-[30%]">
                  ...
                  >
                    ...
                    <p
                      className="font-game text-xl leading-none"
                      style={{ color, textShadow: ri === 0 ? '0 0 10px rgba(242,193,78,0.5)' : 'none' }}
                    >
                      {jugador.puntaje}
                    </p>
                  </div>
        ...
        {resto.length > 0 && (
          <div className="space-y-2">
            {resto.map((j, i) => {
              const esActual = j.id === jugadorActual.id;
              return (
                <div
                  key={j.id}
                ...
                >
                  ...
                  <span
                    className="font-game text-sm flex-shrink-0"
        ...
        {/* Posición del jugador si no está en top 20 */}
        {ranking.length > 0 && !ranking.find((j) => j.id === jugadorActual.id) && (
          <div
          ...
          >
            ...
            <span className="font-game text-sm text-[#F2C14E]">
              {jugadorActual.puntaje}
            </span>
          </div>
        )}

        <p className="text-center text-white/15 text-xs mt-8 pb-6">
          Top 20 en tiempo real
        </p>
      </div>
    </div>
  );
