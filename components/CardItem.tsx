    ...
    <article className="group bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow border border-gray-100 overflow-hidden">
      {card.imagenUrl ? (
        <div className="relative w-full h-44">
          ...
          />
          {cat && (
            <div className="absolute top-0 left-0 right-0 h-1" style={{ backgroundColor: cat.bg }} />
          )}
        </div>
      ) : cat ? (
        <div
          className="relative w-full h-28 flex flex-col items-center justify-center gap-1.5"
          style={{ background: `linear-gradient(135deg, ${cat.bg}22 0%, ${cat.bg}08 100%)` }}
        >
          <div
            className="w-10 h-10 rounded-full flex items-center justify-center"
            style={{ backgroundColor: `${cat.bg}25`, color: cat.bg }}
          >
            <ImageIcon className="w-5 h-5 opacity-70" />
          </div>
          <span className="text-[10px] text-gray-400 px-3 text-center">Sin foto</span>
          <div className="absolute top-0 left-0 right-0 h-1" style={{ backgroundColor: cat.bg }} />
        </div>
      ) : null}
